...
const DeleteStudentAccount = ({setDeleteAccountOpen, student}) => {
  const {getSupervisorAccessTokenFromMemory} = useContext(AuthContext);
  const {t} = useTranslation();
  const navigation = useNavigation();

  async function deleteAccount() {
    const studentUrl = `${API_URL}students/${student}/`;
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert(
          ...
          t('attributes.studentSuccessfullyDeleted'),
        );
        navigation.goBack();
      } else {
  ...
  return (
    <StyledView className="flex-1 items-center justify-center bg-black/20 absolute h-screen w-screen z-50">
      <StyledView
        className={`bg-white p-4 mb-32 pt-12 rounded-sm shadow shadow-zinc-400 border-t-2 border-[#FF3115] mx-5`}>
