        ...
        <StyledView className='flex-1 bg-white justify-between p-4 '>
            <StyledView className='gap-4'>
                ...
                <StyledView className='p-4 flex-row items-center justify-between rounded-[18px] border-[1px] border-[#EDEFF3]'>
                    <StyledText className='text-black text-base font-poppi-medium'>{t("attributes.startDate")}</StyledText>
                ...
                <StyledView className='p-[10px] border-[1px] border-[#EDEFF3] mb-5 rounded-[18px]'>
                    <StyledText className='text-lg font-poppi-semibold text-[#204F50]'>{t("attributes.payments")}</StyledText>
                    {cardSelected ? <StyledView className='flex-row p-[10px] justify-between items-center'>
                        ...
                        <StyledTouchableOpacity onPress={()=>navigation.navigate("ManageCards")}>
                            <StyledText className='text-sm text-[#7658F2] font-poppi-medium'>{t("attributes.mainCheckoutPaymentChange")}</StyledText>
                        </StyledTouchableOpacity>
                    </StyledView>
                        :
                        <StyledTouchableOpacity onPress={()=>navigation.navigate("AddCard")} className='flex-row p-[10px] items-center'>
                            <AddIcon />
                            <StyledText className="text-base ml-2 text-[#7658F2] font-poppi-semibold">{t("attributes.addNewCard")}</StyledText>
                        </StyledTouchableOpacity>
                    }
                </StyledView>

                <StyledView className='flex-row justify-between'>
                    <StyledText className='text-black text-base font-poppi-semibold'>{t("attributes.totalPrice")}:</StyledText>
            ...
            <StyledTouchableOpacity className='rounded-[18px] items-center bg-[#76F5A4] py-[10px]'>
                <StyledText className='text-base text-[#204F50] font-poppi-semibold'>{t("attributes.payNow")}</StyledText>
            </StyledTouchableOpacity>
        </StyledView>
    )
}
